...
const Match = () => {
  ...
  const [answers, setAnswers] = useState<MatcherAnswers | null>(null);

  const handleSubmit = async (a: MatcherAnswers) => {
    setLoading(true);
    setAnswers(a);
    try {
      const { data, error } = await supabase.functions.invoke("broker-matcher", { body: a });
      ...
      <section className="pt-10 pb-24 px-4">
        <div className="max-w-3xl mx-auto text-center mb-10">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-mono tracking-widest uppercase mb-4">
            <Sparkles className="w-3 h-3" /> AI Broker Matcher v2
          </span>
          ...
          <p className="text-sm md:text-base text-muted-foreground max-w-xl mx-auto">
            5 quick questions. Smarter weighted scoring picks the top 3 — with the "why" explained.
          </p>
        </div>
        ...
        {matches === null ? (
          ...
          <MatchResults matches={matches} onReset={() => { setMatches(null); setAnswers(null); }} answers={answers} />
        )}
      </section>
    </MainLayout>
  );
